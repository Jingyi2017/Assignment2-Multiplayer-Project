...
public class TicTacToeBootstrap : MonoBehaviour
{
    ...
    private void BuildUi()
    {
        ...
        rootRect.sizeDelta = new Vector2(660f, 1040f);
        ...
        CreateText(rootPanel.transform, "TitleText", "LAN Tic-Tac-Toe", 34, TextAnchor.MiddleCenter, FontStyle.Bold, 54f);
        ...
        AddLayoutElement(connectionPanel, 0f, 272f);
        ...
        connectionText = CreateText(connectionPanel.transform, "ConnectionText", $"Host IP: {DefaultIpAddress}    Port: {DefaultPort}", 18, TextAnchor.MiddleCenter, FontStyle.Normal, 42f);

        boardPanel = new GameObject("BoardPanel", typeof(RectTransform));
        ...
        AddLayoutElement(boardPanel, 0f, 474f);

        cellButtons = new Button[9];
        cellLabels = new Text[9];
        for (int i = 0; i < 9; i++)
        {
            Button cellButton = CreateButton(boardPanel.transform, $"Cell{i}", string.Empty, 72, 150f);
        ...
        statusText = CreateText(rootPanel.transform, "StatusText", "Choose Host or Client to begin.", 22, TextAnchor.MiddleCenter, FontStyle.Bold, 60f);
        restartButton = CreateButton(rootPanel.transform, "RestartButton", "Restart Match", 22, 56f);
    ...
    private void RefreshUi()
    {
        ...
        bool isConnectedOrListening = manager != null && manager.IsListening;

        if (boardPanel != null)
        {
            boardPanel.SetActive(isConnectedOrListening);
        }
    ...
    private InputField CreateInputField(Transform parent, string name, string defaultValue, bool numericOnly)
    {
        ...
        AddLayoutElement(inputObject, 0f, 50f);
